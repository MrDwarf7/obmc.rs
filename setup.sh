...
process_readme_template() {
  ...
  replace_in_file "$output" '{{REPO_NAME}}' "$REPO_NAME_CLEAN"
...
update_cliff_toml() {
  if [[ -f "cliff.toml" ]]; then
    ...
    # Replace the repo name in the git-cliff postprocessor URL
...
setup_using_jj() {
  ...
  if ask_yes_no "Do you want to initialize with $cmd_bin (recommended for existing remote)?"; then
    ...
    ask_choice "Enter choice (1 or 2): " "1" "2"
